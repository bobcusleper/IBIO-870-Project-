...
tree <- read_csv("../Data/Level_1/TreeDataFine_clean.csv", show_col_types = FALSE)
plot <- read_csv("../Data/Level_1/PlotDataFine_clean.csv", show_col_types = FALSE)
...
results <- foreach(i = 1:length(species), .inorder = TRUE, .options.snow = opts) %dopar% {
  ...
  # Plot labels
  label <- vector(mode = "character", length = 4)
  # label[1] <- "y = a * x^b"
  label[1] <- paste0("Forest: ", temp$loc[1])
...
p <- ggarrange(plots[[1]], plots[[2]], plots[[3]], plots[[4]],
  ...
  annotate_figure(
    top = text_grob("Fine Scale Aboveground Biomass Allometry",
...
plot_mass1 <- plot %>%
  ...
  group_by(spp1) %>%
  summarize(mass.est.ac.kg = round(sum(mass.est.ac.kg) / 135))

plot_mass2 <- plot %>%
  left_join(data2 %>% rename("spp1" = species), by = "spp1") %>%
...
biomass_table <- plot_mass %>%
  rename("FIA SpCode" = spp1, "Uncorrected" = 2, "Corrected" = 3) %>%
  gt() %>%
  tab_header(title = md("Table 3: AGB (kg) / Acre by Species")) %>%
  cols_align(
    align = "center",
    columns = everything()
  )
...
gtsave(biomass_table, "Table3.png", "../Figures/")
...
# Get forest variable
forest <- tree %>% filter(spp %in% species) %>% 
...
for (i in 1:length(species)) {
  ...
  mpo[i] <- moran.tests[[i]]$observed
  mpsd[i] <- moran.tests[[i]]$sd
}

moran_table <- tibble(species, forest, round(mpi, 3), round(mpo, 3), round(mpsd, 3)) %>%
  arrange(mpi) %>%
  rename("FIA SpCode" = species, "Forest" = forest, "P-value" = 3, "Observed" = 4, "Standard Deviation" = 5) %>%
  gt() %>%
  tab_header(title = md("Table 4: Moran's I Results Per Species & Forest Stand")) %>%
  cols_align(
    align = "center",
    columns = everything()
  ) %>%
...
# Exports the Moran table
gtsave(moran_table, "Table4.png", "../Figures/")
...
mantel_table <- tibble(species, forest, round(mtp, 3), round(mtz, 3)) %>%
  arrange(mtp) %>%
  rename("FIA SpCode" = species, "Forest" = forest, "P-value" = 3, "Z-Score" = 4) %>%
  gt() %>%
  tab_header(title = md("Table 5: Mantel Test Per Species & Forest Stand")) %>%
  cols_align(
    align = "center",
    columns = everything()
  ) %>%
...
# Exports the mantel table
gtsave(mantel_table, "Table5.png", "../Figures/")
