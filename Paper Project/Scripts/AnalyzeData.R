...
label2[1] <- "y = a * x^b"
label2[2] <- paste0("a = ", round(a2, 3))
label2[3] <- paste0("b = ", round(b2, 3))
...
p2 <- ggplot() +
  ...
  geom_label(
    aes(
      label = paste(label2[1], label2[2], label2[3], sep = "\n"),
      x = min(x_predicted),
      y = max(y_predicted)
    ),
    label.size = .5,
...
p3 <- ggarrange(p1, p2, ncol = 2, nrow = 1, common.legend = TRUE, legend = "right") %>%
  annotate_figure(plot, fig.lab.pos = "top.left", top = text_grob("Above Ground Biomass Allometry",
    face = "bold", size = 14
  ))

p3

ggsave(plot = p3, "../Figures/Allometry.svg", width = 40, height = 20, units = "cm")
...
spatial_table <- bind_rows(
  tibble("Test" = "Moran's I", "z-score" = moran$observed, "p-value" = moran$p.value),
  tibble("Test" = "Mantel", "z-score" = mantel$z.stat, "p-value" = mantel$p)
) %>%
  gt() %>%
  tab_header(
